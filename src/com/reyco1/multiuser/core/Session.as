package com.reyco1.multiuser.core
{
	...
	public class Session
	{
		...
		public function Session(serverAddress:String, groupName:String)
		{
			...
			groupspec.addIPMulticastAddress("224.255.0.0:6555");
		...
		public function connect(userName:String, userDetails:Object = null):void
		{
			...
			connection.addEventListener(AsyncErrorEvent.ASYNC_ERROR, onErrorHandled);
			connection.addEventListener(IOErrorEvent.IO_ERROR, onErrorHandled);
			connection.addEventListener(SecurityErrorEvent.SECURITY_ERROR, onErrorHandled);
			
			connection.connect( serverAddress );
		}
		
		private function onErrorHandled(e:ErrorEvent):void 
		{
			trace(e.text);
		}
		...
		protected function handleNetStatusEvent(event:NetStatusEvent):void
		{
			...
			switch(event.info.code)
			{
				...
				case "NetConnection.Connect.Closed":					
					P2PDispatcher.instance.dispatchEvent(new UserStatusEvent(UserStatusEvent.DISCONNECTED));					
					break;
			}
		}
